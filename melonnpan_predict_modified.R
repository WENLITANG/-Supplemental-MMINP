...
#' @export
melonnpan.predict.modified<-function(
              metag,
              weight.matrix = NULL,
              train.metag = NULL,
  ...
  if (is.character(metag)){
    test.metag <-data.frame(readTable(metag), check.names = F)
  } else{    
    test.metag<-metag
  }
  ...
  } else if (is.character(train.metag)){ 
  ...
  } else if (is.character(weight.matrix)) { 
